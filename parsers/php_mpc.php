...
foreach ($content as $line) {
	if (substr($line, 0, 1) != "[") {
        ...
        if ($time != '') {
            ...
            // ignore lines wich are not interesting
            // e.g. "Permission denied" or "client denied by server configuration"
            $dummy = [];
            $ignoreLine = false;
            $ignoreLine = $ignoreLine || (preg_match('/\(13\)Permission denied:/', $line, $dummy) === 1);
            $ignoreLine = $ignoreLine || preg_match('/AH01630: client denied by server configuration/', $line, $dummy) === 1;
